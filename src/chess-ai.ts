...
export function makeBestMove({ game, previousSum }: {
  game: ChessInstance
  previousSum: number
}) {
  const color = 'b'
  debug('chess-ai:makeBestMove')('start thinking...')

  const [move] = getBestMove(game, color, previousSum);
  const { san: bestMove } = move
  const currentSum = evaluateBoard(game, move, previousSum, 'b');

  debug('chess-ai:makeBestMove')('done. best move is %o', bestMove)
  return { sum: currentSum, move: bestMove };
}

function getBestMove(game: ChessInstance, color: string, currSum: number) {
  // positionCount = 0;
